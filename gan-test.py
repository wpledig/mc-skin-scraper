...
def make_generator_model():
    ...
    model.add(layers.Dense(8*8*256, use_bias=False, input_shape=(10000,)))
...
def make_discriminator_model():
    model = tf.keras.Sequential()
    model.add(layers.Conv2D(32, (5, 5), strides=(1, 2), padding='same', input_shape=[32, 64, 4]))
    model.add(layers.LeakyReLU())
...
def load_images(directory):
    ...
    for i in range(num):
        filename = files[i]
        cur_img = PIL.Image.open(directory + filename).convert(mode="RGBA")
        cur_img.load()
...
def generate_and_save_images(model, epoch, test_input):
    ...
    gen_image("samples/SAMPLE - epoch #{:04d}".format(epoch))
...
train(train_dataset, EPOCHS, 800)
